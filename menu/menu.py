...
class Menu:
  ...
  def process_menu_commands(self, server, server_fields, join_server, command=str):
    if self.client_state == MenuState.MAIN_MENU:
      self.handle_main_menu_command(command)
    elif self.client_state == MenuState.CREATE_SERVER:
      self.handle_create_server_command(server, join_server, command)
    elif self.client_state == MenuState.JOIN_SERVER:
      self.handle_join_server_command(server_fields, join_server, command)
  ...
  def handle_create_server_command(self, server, join_server, command: str):
    if command is not None:
      subprocess.Popen(['python', 'server_launcher.py', command])
      host_info = server.get_host_info()
      join_server({ 'IP_ADDRESS': host_info['ip_address'], 'PORT': 58008, 'PASSWORD': command })

  def handle_join_server_command(self, server_fields, join_server, command: str):
